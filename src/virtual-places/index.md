...
Implementing these traits requires `unsafe` and handling raw pointers, since we
want to support any kind of pointer expressible in Rust. Using them with the
usual place syntax is safe (given that the smart pointer declares them as
such), since these traits are heavily integrated with the borrow checker.
