...
For a general explanation of autoref and method resolution, see [here](../autoref.md).
...
> [!NOTE]
> The place expression `parent.example` is desugared to `@%MaybeUninit (**parent).example`, which has the type `MaybeUninit<Example>`, see [place expression desugaring](./place-expressions.md). Place expressions are passed to the method resolution algorithm in their desugared form.
